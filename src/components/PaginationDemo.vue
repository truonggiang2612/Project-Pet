...
<template>
    <div class="flex flex-col justify-center">
      <div class="flex justify-start items-center mb-[20px] sm:mt-[100px]">
          <input v-model="textValue" type="text" class="border-2 h-[30px] rounded mr-[20px] w-[200px] shadow-[0_5px_10px_5px_rgba(0,0,0,0.3)] md:w-[300px] md:h-[40px]">
          <button @click="postData" class="border-2 p-[2px] rounded active:bg-[#0000ff] shadow-[0_5px_10px_5px_rgba(0,0,0,0.3)] md:p-[6px]">Add</button>
      ...
      <div class="mb-[10px]" v-for="(user, index) in filteredList" :key="index">
        <div class="w-full">
          ...
          <button @click="deletePost(index)" class="border-2 rounded p-[2px] text-[14px] active:bg-[#ff0000] shadow-[0_5px_10px_5px_rgba(0,0,0,0.3)] md:p-[2px]">Delete</button>
        </div>
      </div>

      <div class="w-50% flex justify-center items-center mt-[20px]">
        <button class="border-2 p-[4px] rounded-[50%] mr-[30px] hover:bg-[#ffcdd2] active:bg-[#f44336] active:border-[#000] md:p-[6px]" :disabled="currentPage === 1" @click="changePage(-1)">Prev</button>
        <div>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 12c0-1.232-.046-2.453-.138-3.662a4.006 4.006 0 00-3.7-3.7 48.678 48.678 0 00-7.324 0 4.006 4.006 0 00-3.7 3.7c-.017.22-.032.441-.046.662M19.5 12l3-3m-3 3l-3-3m-12 3c0 1.232.046 2.453.138 3.662a4.006 4.006 0 003.7 3.7 48.656 48.656 0 007.324 0 4.006 4.006 0 003.7-3.7c.017-.22.032-.441.046-.662M4.5 12l3 3m-3-3l-3 3" />
          </svg>
        </div>
        <button class="border-2 p-[4px] rounded-[50%] ml-[30px] hover:bg-[#ffcdd2] active:bg-[#f44336] active:border-[#000] md:p-[6px]" :disabled="currentPage === 10" @click="changePage(1)">Next</button>
      </div>
    </div>
</template>
